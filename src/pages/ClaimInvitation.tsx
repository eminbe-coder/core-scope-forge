...
export default function ClaimInvitation() {
  ...
  const handleLoginAndLink = async (email: string, password: string) => {
    setIsLoading(true);
    try {
...
function LinkLoginForm({ onBack, onLogin, isLoading, invitedEmail }: LinkLoginFormProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Sign in to your existing account to link the invitation for <strong>{invitedEmail}</strong>.
      </p>
